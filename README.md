...
| FX constructs (.fx) | Complete | Techniques/passes captured with shader bindings (FX9 `compile` and FX10 `Set*Shader(CompileShader())` forms), render/state blocks, and diagnostics for missing VS/PS bindings, duplicate techniques/passes, and profile mismatches; shared/pool/state objects surface as symbols |
