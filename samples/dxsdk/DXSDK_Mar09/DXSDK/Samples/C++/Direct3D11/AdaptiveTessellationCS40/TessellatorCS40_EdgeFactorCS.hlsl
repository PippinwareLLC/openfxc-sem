...
RWStructuredBuffer<float> EdgeFactorBufOut : register(u0);
...
void CSEdgeFactor( uint3 DTid : SV_DispatchThreadID )
{
    ...
    {
        factor.x = length((p1.xy - p0.xy) * g_tess_edge_length_scale);
        factor.y = length((p2.xy - p1.xy) * g_tess_edge_length_scale);
        factor.z = length((p0.xy - p2.xy) * g_tess_edge_length_scale);
    ...
    EdgeFactorBufOut[DTid.x*3+0] = factor.x;
    EdgeFactorBufOut[DTid.x*3+1] = factor.y;
    EdgeFactorBufOut[DTid.x*3+2] = factor.z;
}