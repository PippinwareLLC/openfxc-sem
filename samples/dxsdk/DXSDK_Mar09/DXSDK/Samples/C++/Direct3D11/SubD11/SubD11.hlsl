...
static const float g_fSpecularIntensity = 0.3f;
...
float2 ComputeDirectionalLight( float3 vWorldPos, float3 vWorldNormal, float3 vDirLightDir )
{
    ...
    Result.x = saturate( dot( vWorldNormal, -vDirLightDir ) );
...
float4 SmoothPS( PS_INPUT Input ) : SV_TARGET
{
    ...
    const float3 DirLightDirections[3] =
    {
        // key light
        normalize( float3( -5.0f, -2.0f, 5.0f ) ),
        // fill light
        normalize( float3( 5.0f, -2.0f, 5.0f ) ),
        // back light
        normalize( float3( -2.0f, -2.0f, -5.0f ) ),
    };
    
    const float3 DirLightColors[3] = 
    {
        // key light
        float3( 1.0f, 1.0f, 0.8f ) * 1.0f,
        // fill light
        float3( 0.8f, 0.8f, 1.0f ) * 0.5f,
        // back light
        float3( 0.8f, 0.8f, 1.0f ) * 0.5f,
    };
