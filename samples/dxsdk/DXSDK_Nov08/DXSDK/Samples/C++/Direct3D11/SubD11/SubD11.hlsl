...
VS_CONTROL_POINT_OUTPUT SkinningVS( VS_CONTROL_POINT_INPUT Input )
{
...
float ComputeSpot( float3 vWorldPos, float3 vWorldNormal, float4 vSpotLightPosAngle, float3 vSpotLightDir )
{
    float SpotDirection = pow( saturate( dot( vWorldNormal, -vSpotLightDir ) ), 6 );

    float3 vSpotToPixel = vWorldPos - vSpotLightPosAngle.xyz;
    float Len = length( vSpotToPixel );
    vSpotToPixel /= Len;
    
    const float SpotAngle = vSpotLightPosAngle.w;
    float PixelAngle = dot( vSpotLightDir, vSpotToPixel );
    float Falloff = saturate( PixelAngle - SpotAngle ) / ( 1 - SpotAngle );
    return Falloff * SpotDirection;
}

float4 SmoothPS( PS_INPUT Input ) : SV_TARGET
{
    float4 vNormalMapSampleRaw = g_txHeight.Sample( g_samLinear, Input.vUV ); 
    float3 vNormalMapSample = normalize( ( vNormalMapSampleRaw.xyz * float3( 2, 2, 2 ) ) - float3( 1, 1, 1 ) );
    ...
    float4 vColor = float4( 0.5, 0.5, 0.5, 1 );
    
    const float4 SpotLightPositionsAngles[4] =
    {
        float4( 19.399, 25.904, -8.465, 0.3681 ),
        float4( -18.012, 29, -3.222, 0.7660 ),
        float4( -12.525, 18.62, 25.759, 0.4226 ),
        float4( 21.08, 11.543, 11.805, 0.4226 )
    };
    
    const float3 SpotLightColors[4] =
    {
        float3( 1, 0.9, 0.58 ) * 0.8,
        float3( 1, 0.8, 0.75 ) * 0.5,
        float3( 1, 0.99, 0.89 ) * 1.2,
        float3( 0.25, 0.85, 1 ) * 0.6,
    };
    
    float3 fLighting = 0;
    for( int i = 0; i < 4; ++i )
    {
        float fSpotLight = ComputeSpot( Input.vWorldPos, vNormal, SpotLightPositionsAngles[i], normalize( -SpotLightPositionsAngles[i].xyz ) );
        fLighting += fSpotLight * SpotLightColors[i];
    }
    
    return float4( fLighting, 1 ) * vColor;
}

//--------------------------------------------------------------------------------------
// Solid color shading pixel shader section (used for wireframe overlay)
//--------------------------------------------------------------------------------------
